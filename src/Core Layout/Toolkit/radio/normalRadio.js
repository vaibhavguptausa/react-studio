...
const normalRadioSource = {
    beginDrag(props) {
    const {type, id, positionX, positionY } = props
      return {type,id, positionX, positionY};
    }
  };
 class NormalRadio extends Component {
     constructor(props)
     {
         super(props);
         this.state = { modalState: false, height: 50, width: 70, color: 'yellow', type: 'radio'};
     }
     handleClick = (e) => {
      if (e.type === 'click') {
  ...
  handleChangeAttributes = (parameters) => {
    ...
    export default DragSource(ItemTypes.NORMALRADIO, normalRadioSource, (connect, monitor,Component) => ({
